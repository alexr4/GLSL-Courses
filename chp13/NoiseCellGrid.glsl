...
void main(){
  ...
  float randPerCell = noise(ist * 0.5 + inc);//get an random for each cell depending on their indice and increment
  //randPerCell = smoothstep(0.5, 0.65, randPerCell);//try this in order get a nice bit pattern. The main idea is to check is the random is equal or great than a specific value to define if we draw the cell or not
